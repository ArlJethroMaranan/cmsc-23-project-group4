...
class EditDonationDrive extends StatefulWidget {
  ...
  final DonationDriveModel
      drive; // call final drive so I can access the values in the drive
...
class _EditDonationDriveState extends State<EditDonationDrive> {
  ...
  void validateAnswers() {
    ...
    summaryText = 'Name: ${nameController.text}\n'
    ...
    if (!formKey.currentState!.validate() || summaryText.contains('null')) {
      ...
      formKey.currentState!.save();
      {
        ...
        context.read<DonationDriveOrgListProvider>().editDrive(widget.drive.id!,
            temp); // call id from drive and temp holding the values to edit
  ...
  @override
  void initState() {
    // set initial state to the old infos when editing
    super.initState();
    nameController = TextEditingController(text: widget.drive.name!);
    descriptionController =
        TextEditingController(text: widget.drive.description!);
