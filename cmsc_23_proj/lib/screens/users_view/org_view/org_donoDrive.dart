...
class _DonationDriveState extends State<DonationDrive> {
  ...
  Widget build(BuildContext context) {
    Stream<QuerySnapshot> donationDriveOrgStream =
        context.watch<DonationDriveOrgListProvider>().donationDriveOrg;
          ...
          builder: (context, snapshot) {
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                Map<String, dynamic> docMap = snapshot.data!.docs[index].data()
                    as Map<String,
                        dynamic>; // store to docMap the data from firebase

                docMap['id'] = snapshot.data!.docs[index]
                    .id; // store to id in model the id from snapshot (will be used for editing)

                // store to temp the data from snapshot
                DonationDriveModel temp = DonationDriveModel.fromJson(docMap);
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                    temp.description!.length > 15
                                        ? temp.description!.substring(0, 15)
                                        : "${temp.description}",
                                    style: const TextStyle(
                                        fontSize:
                                            20)), // make sure that it shows only up to 15 length so that it does not overflow
                              ],
                            ),
                          ),
                          Column(
                                ...
                                child: FloatingActionButton.extended(
                                  onPressed: () async {
                                    context
                                        ...
                                        .updateSelectedDrive(
                                            temp); // pass donation drive(temp) to update selected drive
                                    await Navigator.pushNamed(
                                        context, '/driveOrgDet');
                                  },
                                  backgroundColor: const Color.fromARGB(
                                      255, 229, 239, 95), // Background color
                                  label: const Text(''),
